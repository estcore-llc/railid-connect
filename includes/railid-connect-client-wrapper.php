...
/**
 * Плагин аутентификации OIDC/oAuth обложка класса клиента.
 *
...
/**
 * RailID_Connect_Client_Wrapper class.
 *
 * Плагин аутентификации OIDC/oAuth обложка класса клиента.
 *
 * @package  Estcore_RailID_Connect
 * @category Authentication
 */
class Estcore_RailID_Connect_Client_Wrapper {

	private $client;
	private $settings;
	private $logger;
	private $cookie_token_refresh_key = 'railid-connect-refresh';
	public $cookie_redirect_key = 'railid-connect-redirect';
	private $error = false;

	/**
	 * Inject necessary objects and services into the client.
	 *
	 * @param RailID_Connect_Client          $client   A plugin client object instance.
	 * @param RailID_Connect_Option_Settings $settings A plugin settings object instance.
	 * @param RailID_Connect_Option_Logger   $logger   A plugin logger object instance.
	 */
	public function __construct( RailID_Connect_Client $client, RailID_Connect_Option_Settings $settings, RailID_Connect_Option_Logger $logger ) {
		$this->client = $client;
	 ...
	 *
	 * @param \RailID_Connect_Client          $client   The plugin client instance.
	 * @param \RailID_Connect_Option_Settings $settings The plugin settings instance.
	 * @param \RailID_Connect_Option_Logger   $logger   The plugin logger instance.
	 *
	 * @return \RailID_Connect_Client_Wrapper
	 */
	public static function register( RailID_Connect_Client $client, RailID_Connect_Option_Settings $settings, RailID_Connect_Option_Logger $logger ) {
		...
		add_filter( 'railid-connect-alter-request', array( $client_wrapper, 'alter_request' ), 10, 3 );
		...
		if ( is_admin() ) {
			 ...
			 */
			add_action( 'wp_ajax_railid-connect-authorize', array( $client_wrapper, 'authentication_request_callback' ) );
			add_action( 'wp_ajax_nopriv_railid-connect-authorize', array( $client_wrapper, 'authentication_request_callback' ) );
		}

		if ( $settings->alternate_redirect_uri ) {
			// Provide an alternate route for authentication_request_callback.
			add_rewrite_rule( '^railid-connect-authorize/?', 'index.php?railid-connect-authorize=1', 'top' );
			add_rewrite_tag( '%railid-connect-authorize%', '1' );
	...
	public function alternate_redirect_uri_parse_request( $query ) {
		if ( isset( $query->query_vars['railid-connect-authorize'] ) &&
			 '1' === $query->query_vars['railid-connect-authorize'] ) {
	...
	public function get_redirect_to() {
		...
		$redirect_url = apply_filters_deprecated(
			'railid-connect-cookie-redirect-url',
			array( $redirect_url ),
			'1.0.0',
			'railid-connect-client-redirect-to'
		);

		// This is the new hook to use with the transients version of redirection.
		return apply_filters( 'railid-connect-client-redirect-to', $redirect_url );
	}
	...
	public function get_authentication_url( $atts = array() ) {
		...
		$this->logger->log( apply_filters( 'railid-connect-auth-url', $url ), 'make_authentication_url' );
		return apply_filters( 'railid-connect-auth-url', $url );
	}
	...
	public function ensure_tokens_still_fresh() {
		...
		if ( ! $refresh_token || ( $refresh_expires && $current_time > $refresh_expires ) ) {
			...
			if ( $this->settings->redirect_on_logout ) {
				$this->error_redirect( new WP_Error( 'access-token-expired', __( 'Session expired. Please login again.', 'estcore-railid-connect' ) ) );
			}
	...
	public function get_end_session_logout_redirect_url( $redirect_url, $requested_redirect_to, $user ) {
		...
		$token_response = $user->get( 'railid-connect-last-token-response' );
		if ( ! $token_response ) {
		...
		$claim = $user->get( 'railid-connect-last-id-token-claim' );
	...
	public function authentication_request_callback() {
		...
		$token_response = apply_filters( 'railid-connect-modify-token-response-before-validation', $token_response );
		...
		// Allow for other plugins to alter data before validation.
		$id_token_claim = apply_filters( 'railid-connect-modify-id-token-claim-before-validation', $id_token_claim );
		...
		if ( ! $user ) {
			if ( $this->settings->create_if_does_not_exist ) {
				...
				$this->error_redirect( new WP_Error( 'identity-not-map-existing-user', __( 'User identity is not linked to an existing WordPress user.', 'estcore-railid-connect' ), $user_claim ) );
			}
		} else {
			// Allow plugins / themes to take action using current claims on existing user (e.g. update role).
			do_action( 'railid-connect-update-user-using-current-claim', $user, $user_claim );
		}
		...
		do_action( 'railid-connect-user-logged-in', $user );
		...
		$state_object = get_transient( 'railid-connect-state--' . $state );
		...
		if ( $this->settings->redirect_user_back ) {
			do_action( 'railid-connect-redirect-user-back', $redirect_url, $user );
		}
	...
	public function validate_user( $user ) {
		...
		if ( ! is_a( $user, 'WP_User' ) || ! $user->exists() ) {
			return new WP_Error( 'invalid-user', __( 'Invalid user.', 'estcore-railid-connect' ), $user );
		}
	...
	public function login_user( $user, $token_response, $id_token_claim, $user_claim, $subject_identity ) {
		...
		update_user_meta( $user->ID, 'railid-connect-last-token-response', $token_response );
		update_user_meta( $user->ID, 'railid-connect-last-id-token-claim', $id_token_claim );
		update_user_meta( $user->ID, 'railid-connect-last-user-claim', $user_claim );
	...
	public function get_user_by_identity( $subject_identity ) {
		// Look for user by their railid-connect-subject-identity value.
		$user_query = new WP_User_Query(
			array(
				'meta_query' => array(
					array(
						'key'   => 'railid-connect-subject-identity',
	...
	private function get_username_from_claim( $user_claim ) {
		...
		if ( ! empty( $this->settings->identity_key ) && isset( $user_claim[ $this->settings->identity_key ] ) ) {
			...
			return new WP_Error( 'no-username', __( 'No appropriate username found.', 'estcore-railid-connect' ), $user_claim );
		}
		...
		if ( empty( $transliterated_username ) ) {
			...
			return new WP_Error( 'username-transliteration-failed', sprintf( __( 'Username %1$s could not be transliterated.', 'estcore-railid-connect' ), $desired_username ), $desired_username );
		}
		...
		if ( empty( $normalized_username ) ) {
			...
			return new WP_Error( 'username-normalization-failed', sprintf( __( 'Username %1$s could not be normalized.', 'estcore-railid-connect' ), $transliterated_username ), $transliterated_username );
		}
	...
	private function get_nickname_from_claim( $user_claim ) {
		...
		if ( empty( $desired_nickname ) ) {
			...
			return new WP_Error( 'no-nickname', sprintf( __( 'No nickname found in user claim using key: %1$s.', 'estcore-railid-connect' ), $this->settings->nickname_key ), $this->settings->nickname_key );
		}
				...
				if ( ! isset( $user_claim[ $key ] ) ) {
					if ( $error_on_missing_key ) {
						return new WP_Error(
							...
							__( 'User claim incomplete.', 'estcore-railid-connect' ),
							array(
	...
	/**
	 * Создайте нового пользователя на основе сведений в user_claim.
	 *
	 * @param string $subject_identity Идентификатор аутентифицированного пользователя с IDP.
	 * @param array  $user_claim       Заявление авторизованного пользователя.
	 *
	 * @return \WP_Error | \WP_User
	 */
	public function create_new_user( $subject_identity, $user_claim ) {
		$user_claim = apply_filters( 'railid-connect-alter-user-claim', $user_claim );
		...
		if ( $values_missing && isset( $user_claim['access_token'] ) && ! empty( $this->settings->endpoint_userinfo ) ) {
			...
			if ( is_wp_error( $user_claim_result ) ) {
				return new WP_Error( 'bad-user-claim-result', __( 'Bad user claim result.', 'estcore-railid-connect' ), $user_claim_result );
			}
		...
		$create_user = apply_filters( 'railid-connect-user-creation-test', true, $user_claim );

		if ( ! $create_user ) {
			return new WP_Error( 'cannot-authorize', __( 'Can not authorize.', 'estcore-railid-connect' ), $create_user );
		}
		...
		$user_data = apply_filters( 'railid-connect-alter-user-data', $user_data, $user_claim );
		$uid = wp_insert_user( $user_data );
		if ( is_wp_error( $uid ) ) {
			return new WP_Error( 'failed-user-creation', __( 'Failed user creation.', 'estcore-railid-connect' ), $uid );
		}
		$user = get_user_by( 'id', $uid );
		add_user_meta( $user->ID, 'railid-connect-subject-identity', (string) $subject_identity, true );
		$this->logger->log( "New user created: {$user->user_login} ($uid)", 'success' );
		do_action( 'railid-connect-user-create', $user, $user_claim );

		return $user;
	}

	/**
	 * Обновить существующего пользователя с помощью метаданных RailID Connect.
	 *
	 * @param int    $uid              Идентификатор пользователя WordPress.
	 * @param string $subject_identity Личность субъекта от IDP.
	 *
	 * @return WP_Error|WP_User
	 */
	public function update_existing_user( $uid, $subject_identity ) {
		update_user_meta( $uid, 'railid-connect-subject-identity', strval( $subject_identity ) );
		do_action( 'railid-connect-user-update', $uid );
